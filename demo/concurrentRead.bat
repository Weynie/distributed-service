...
REM This instance runs a sequence: select E5 select E2.
REM After finishing, it echoes "done" into client2_done.txt and then pauses.
start "Client2" cmd /k "java -cp target/classes com.distributed.service.Client select E5 select E2 & echo done > client2_done.txt & pause"
